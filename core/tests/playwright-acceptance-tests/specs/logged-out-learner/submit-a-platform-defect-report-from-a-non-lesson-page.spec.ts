...
test.describe('Logged-Out User', function () {
  ...
  test('should scroll down to the bottom global footer layout and click the "Report a Website Issue" link.', async () => {
    ...
    await loggedOutLearner.expectScreenshotToMatch('reportASiteIssueModal');
  ...
  test('should not be able to submit "Report a Website Issue" feedback while the text area description is completely blank.', async () => {
    await loggedOutLearner.openReportASiteIssueModalFromGlobalFooter(false);
    await loggedOutLearner.scrollToCaptchaContainer();
    await loggedOutLearner.clickButtonInModal(
      'Report a Website Issue',
